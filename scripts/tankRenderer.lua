...
local barSize = 17	--Tamanho em graus da barra dos indicadores de potencia e vida
...
function TankRenderer:draw(t)
	...
	love.graphics.arc("line", "open", gameCenter.x, gameCenter.y, gameArena.raio+30, t.tank.pos-math.rad(barSize/2), t.tank.pos-math.rad(barSize/2) + map(t.status.holdtime, 0, 3, 0, math.rad(barSize)))
	...
	for i=0,t.status.life-1 do
		...
		love.graphics.arc("line", "open", gameCenter.x, gameCenter.y, gameArena.raio+15, t.tank.pos-math.rad(i * lifeBarSpacing - barSize/2), t.tank.pos-math.rad(i * lifeBarSpacing - barSize/2 + singleLifeRad))	
	end
